...
BEGIN
    ...
    -- OBTENER LA FECHA DE FIN DE INSCRIPCIÓN PARA LA COMPETICIÓN
    SELECT FECHA_INICIO INTO V_FECHA_FIN
    FROM COMPETICIONES
    WHERE COD_COMPE = V_COD_COMPE;

    -- VERIFICAR SI LA FECHA DE FIN DE INSCRIPCIÓN HA PASADO
    IF V_FECHA_FIN < SYSDATE THEN
        DBMS_OUTPUT.PUT_LINE('LA ETAPA DE INSCRIPCIÓN PARA LA COMPETICIÓN ESTÁ CERRADA.');
    ELSE
        DBMS_OUTPUT.PUT_LINE('LA ETAPA DE INSCRIPCIÓN PARA LA COMPETICIÓN ESTÁ ABIERTA.');
    END IF;
END;
