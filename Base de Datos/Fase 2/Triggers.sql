...
COMPOUND TRIGGER
    ...
    V_COD_EQUIPO_NEW NUMBER; -- C�digo de equipo para nueva inserci�n o actualizaci�n
BEFORE EACH ROW IS
    BEGIN
...
AFTER STATEMENT IS
    BEGIN
        -- Calcular el sueldo total del equipo despu�s de insert o update
        ...
        IF SUELDO_TOTAL > 200000 THEN
            RAISE_APPLICATION_ERROR(-20004, 
                'Sobrepasa el l�mite salarial para jugadores');
        END IF;
    END AFTER STATEMENT;
END;
/
...
AFTER STATEMENT IS
    BEGIN
        -- Contar el n�mero de entrenadores 
        ...
        IF INSERTING THEN
            ...
            END IF;
        END IF;
        -- Verificar si despu�s del ajuste no hay ning�n entrenador
...
FOR INSERT OR UPDATE ON STAFF
COMPOUND TRIGGER
    -- Declaraci�n de variables a nivel del trigger
...
BEFORE EACH ROW IS
    BEGIN
        ...
        V_COD_EQUIPO := :NEW.COD_EQUIPO;
        -- Capturar el cargo anterior si es una actualizaci�n
        IF UPDATING THEN
            V_CARGO_ANTERIOR := :OLD.CARGO;
        ELSE
            V_CARGO_ANTERIOR := NULL;
...
AFTER STATEMENT IS
    BEGIN
        -- Contar el n�mero de asistentes
        ...
        IF UPDATING THEN
            ...
            END IF;
        END IF;
        -- Verificar si hay m�s de un asistente
...
BEGIN
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            EN_CURSO := 0;
        WHEN OTHERS THEN
            RAISE_APPLICATION_ERROR(-20009, 'Error desconocido al verificar la competici�n');
    END;
...
FOR EACH ROW
DECLARE
    EN_CURSO NUMBER;  -- Para verificar si la competici�n esta en curso
BEGIN
    -- Verificar si el equipo esta en una competici�n en curso
    ...
    AND C.CURSO = 1;
    -- Si la competicion est� en curso, bloquear la operacion
    IF EN_CURSO > 0 THEN
...
BEGIN
    -- Verificar si el equipo est� en una competici�n en curso
    SELECT COUNT(*) INTO EN_CURSO
    FROM COMPETICIONES C
    JOIN EQUIPO_COMPETICION EC ON EC.COD_COMPETICION = C.COD_COMPE
    JOIN EQUIPOS E ON EC.COD_EQUIPO = E.COD_EQUIPO
